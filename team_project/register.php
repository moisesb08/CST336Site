...
<body>
	...
	<form method="post">
		...
		<table>
		<tr>
		<td><p class="required">First Name: </p></td><td><input class="longInput" type='text' name='FirstName' placeholder='First Name' <?php if($retry) echo'value="'.$_POST["FirstName"].'"'?> maxLength='25' required></td>
		<td><p class="required">Last Name: </td><td><input class="longInput" type='text' name='LastName' placeholder='Last Name' <?php if($retry) echo'value="'.$_POST["LastName"].'"'?> maxLength='25' required></td>
		</tr>
		<tr>
		<td><p class="required">Username: </td><td><input class="longInput" type='text' name='CustomerUsername' <?php if($retry) echo'value="'.$_POST["CustomerUsername"].'"'?> minLength='5' maxLength='20' placeholder='Username' required></td>
		<td><p class="required">Email: </td><td><input class="longInput" type='email' name='Email' <?php if($retry) echo'value="'.$_POST["Email"].'"'?> placeholder='Email' maxLength='100' required></td>
		</tr>
		<tr>
		<td><p class="required">Phone: </td><td colspan="3">(<input type='text' class="phone3" name='Phone1' <?php if($retry) echo'value="'.$_POST["Phone1"].'"'?> placeholder='555' minLength='3' maxLength='3' size='1' required>)
		<input type='text' class="phone3" name='Phone2' <?php if($retry) echo'value="'.$_POST["Phone2"].'"'?> placeholder='555' minLength='3' maxLength='3' size='1' required>
		-<input type='text' class="phone4" name='Phone3' <?php if($retry) echo'value="'.$_POST["Phone3"].'"'?> placeholder='5555' minLength='4' maxLength='4' size='2' required></td>
		</tr>
		<tr>
